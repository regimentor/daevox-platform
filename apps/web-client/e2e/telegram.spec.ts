...
test('real HTTP QR flow across two tabs, 2FA, logout and reconnection', async ({
  ...
  expect((await stats()).connects - before.connects).toBe(1);
  ...
  await expect(page.getByRole('button', { name: 'Настройки аккаунта' })).toBeVisible();
  await expect(second.getByRole('button', { name: 'Настройки аккаунта' })).toBeVisible();
  ...
  expect((await stats()).passwords - before.passwords).toBe(1);
  ...
  await page.keyboard.press('Escape');
  expect((await stats()).logouts - before.logouts).toBe(0);
  await page.getByRole('button', { name: 'Настройки аккаунта' }).click();
  await page.getByRole('button', { name: 'Отключить аккаунт' }).click();
  await page.getByRole('button', { name: 'Подтвердить отключение', exact: true }).click();
  await expect(page.getByRole('button', { name: 'Подключить Telegram' })).toBeEnabled();
  expect((await stats()).logouts - before.logouts).toBe(1);
  expect((await stats()).generations - before.generations).toBe(1);
