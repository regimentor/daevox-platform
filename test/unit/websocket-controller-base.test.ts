...
import { InvalidWebSocketControllerError } from '../../lib/framework/errors.ts';
...
test('WebSocketControllerBase предоставляет зависимости прямому наследнику', () => {
  ...
  const jobRunner: Record<string, any> = {};
  const events = { push() {} };
  const controller = new EventsController({ jobRunner, events } as any);
...
test('WebSocketControllerBase строго проверяет options', () => {
  class EventsController extends WebSocketControllerBase {}
  const options = { jobRunner: { run() {}, close() {} }, events: { push() {} } };
  for (const value of [undefined, null, {}, { jobRunner: {} }, { ...options, extra: true }]) {
    assert.throws(() => new EventsController(value as any), InvalidWebSocketControllerError);
  }
});
