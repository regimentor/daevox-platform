...
import { WebSocketSender } from '../../lib/framework/WebSocketSender.js';
...
test('WebSocketSender строго проверяет target', () => {
  ...
  for (const target of invalidTargets) {
    assert.throws(() => sender.send(target, message), InvalidWebSocketSendError);
  }
});
...
test('WebSocketSender строго проверяет envelope сообщения', () => {
  ...
  for (const invalidMessage of invalidMessages) {
    assert.throws(
      () => sender.send({ clientId: 'client-1' }, invalidMessage),
      InvalidWebSocketSendError,
    );
  }
});
