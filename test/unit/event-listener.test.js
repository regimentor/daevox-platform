...
test('EventListenerBase предоставляет listener ровно jobRunner и websocket', () => {
  ...
  const jobRunner = {};
  const websocket = {};
  const listener = new Listener({ jobRunner, websocket });
  ...
  for (const value of [undefined, null, {}, { jobRunner }, { jobRunner, websocket, extra: true }]) {
    assert.throws(() => new Listener(value), InvalidEventListenerError);
  }
});

test('Application регистрирует корректный EventListener до запуска', async (t) => {
  class Created {}
...
test('registerEventListener атомарно отклоняет повтор класса, имени и адреса', async (t) => {
  class Data {}
...
test('registerEventListener запрещён после начала listen', async (t) => {
  class Data {}
...
test('Application строго проверяет секцию events', () => {
  ...
  for (const events of invalidSections) {
    assert.throws(() => new Application({ events }), InvalidEventOptionsError);
  }
});
...
test('HTTP-контроллер fire-and-forget передаёт исходный DTO адресованному listener', async (t) => {
  let releaseHandler;
