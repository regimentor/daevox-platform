...
import { HttpRouter } from '../../lib/framework/HttpRouter.ts';
import {
...
test('HttpRouter отклоняет разреженную пачку как неверный HTTP-маршрут', () => {
  ...
  assert.throws(() => router.registerAll(routes as any), InvalidHttpRouteError);
  assert.equal(router.match('GET', '/users'), null);
});
...
test('HttpRouter отклоняет неверные аргументы и pathname', () => {
  ...
  for (const definitions of [
    ...
    assert.throws(() => router.registerAll(definitions as any), InvalidHttpRouteError);
  }
