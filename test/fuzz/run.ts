...
class SocketObserver {
  ...
  waitFor(predicate: any, timeout: any, label: any, reportedTimeout: any = timeout) {
    return new Promise<any>((resolve: any, reject: any) => {
      ...
      const finish = (settle: any, value: any) => {
        clearTimeout(timer);
...
function parseServerFrames(bytes: any) {
  const frames: any[] = [];
  let offset = 0;
...
function observedWebSocketOutcome(bytes: any, closed: any) {
  const frames = parseServerFrames(bytes);
  const close = frames.find((frame: any) => frame.opcode === 8);
  const texts = frames.filter((frame: any) => frame.opcode === 1);
  const pongs = frames.filter((frame: any) => frame.opcode === 10);
  return {
    closed,
...
function websocketSatisfied(expectation: any, outcome: any) {
...
function assertWebSocketOutcome(expectation: any, outcome: any) {
  ...
  if (expectation.pongPayloads) {
    assert.deepEqual(
      outcome.pongs.map((frame: any) => frame.payload.toString()),
      expectation.pongPayloads,
      'unexpected pong payloads',
    );
  }
  if (expectation.errorCode) {
    const messages = outcome.texts.map((frame: any) => JSON.parse(frame.payload.toString()));
    assert.ok(
      messages.some((message: any) => message.body?.error?.code === expectation.errorCode),
      `missing recoverable ${expectation.errorCode} response`,
    );
    assert.ok(
      messages.some((message: any) => message.body?.value === 'ok'),
      'connection did not process the valid message after a recoverable error',
    );
  }
}

function remainingTime(deadline: any, totalTimeout: any, label: any) {
  const remaining = deadline - Date.now();
  if (remaining <= 0) throw new Error(`${label} exceeded ${totalTimeout}ms`);
  return remaining;
}

async function connect(address: any, sockets: any, timeout: any, reportedTimeout: any = timeout) {
  const socket = net.connect(address.port, address.address);
  sockets.add(socket);
  socket.once('close', () => sockets.delete(socket));
  const observer = new SocketObserver(socket);
  await observer.waitFor(
    (_bytes: any, _closed: any, error: any) => {
...
async function writeChunks(socket: any, chunks: any, testCase: any, limits: any, deadline: any) {
  const operationCount = chunks.reduce((total: any, chunk: any) => {
...
async function executeWebSocketCase(address: any, testCase: any, limits: any, sockets: any) {
  ...
  try {
    ...
    const headerEnd = await observer.waitFor(
      (bytes: any, closed: any, error: any) => {
        if (error) throw error;
...
async function executeHttpCase(address: any, testCase: any, limits: any, sockets: any) {
  const deadline = Date.now() + limits.caseTimeout;
  const { observer, socket } = await connect(
    address,
    sockets,
...
async function waitForNoSockets(sockets: any, timeout: any) {
  const started = Date.now();
  while (sockets.size > 0 && Date.now() - started < timeout) {
    await new Promise<any>((resolve: any) => setTimeout(resolve, 5));
  }
  assert.equal(sockets.size, 0, 'client connections grew between fuzz cases');
}

async function healthCheck(address: any, limits: any, sockets: any) {
...
function applyInjection(testCase: any, injection: any) {
...
export async function runFuzz(options: any = {}) {
  ...
  try {
    for (let caseIndex = 0; caseIndex < cases.length; caseIndex += 1) {
      ...
      assert.ok(
        testCase.chunks.reduce((total: any, chunk: any) => total + chunk.byteLength, 0) <=
          64 * 1024,
      ...
      try {
        ...
        if (asynchronousErrors.length > 0) {
          throw new AggregateError(
            asynchronousErrors.map((entry: any) => entry.error),
            `process observed ${asynchronousErrors.map((entry: any) => entry.type).join(', ')}`,
          );
        }
        completed += 1;
      } catch (error: any) {
