...
function randomJsonObject(random, depthLimit) {
  const remainingNodes = { value: 8 };
  const createValue = (depth) => {
...
function ws(name, frames, expectation, options = {}) {
  return { name, protocol: 'websocket', chunks: frames, expectation, ...options };
}

function http(name, rawRequest, expectation, options = {}) {
  return {
    name,
    protocol: 'http',
...
export function fixedCorpus(bodyLimit = 256) {
  ...
  const closeWith = (payload) => clientFrame({ opcode: 8, payload });
  ...
  return [
    ...
    ws(
      ...
      [...validFrame].map((byte) => Buffer.from([byte])),
      {
        textMessages: 1,
      },
    ),
...
export function generatedCases(seed, count, bodyLimit = 256, depthLimit = 4) {
...
export function serializeCase(testCase) {
  return {
    ...testCase,
    chunks: testCase.chunks.map((chunk) => chunk.toString('base64')),
  };
}

export function deserializeCase(value) {
  return { ...value, chunks: value.chunks.map((chunk) => Buffer.from(chunk, 'base64')) };
}
