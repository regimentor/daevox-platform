...
import { isMain, runModule, score, selectChangedModules, writeReports } from './harness.js';
...
async function main() {
  ...
  const results = moduleReports.flatMap((module) => module.results);
  const report = {
    schemaVersion: 1,
    mode,
  ...
  const unresolved = results.filter((result) => result.status === 'survived');
