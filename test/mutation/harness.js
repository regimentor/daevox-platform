...
export function score(results) {
  ...
  const detected = results.filter((result) => ['killed', 'timeout'].includes(result.status)).length;
  return Number(((detected / results.length) * 100).toFixed(2));
}

export function selectChangedModules(modules, changedPaths) {
  const changed = new Set(changedPaths);
  return modules.filter(
    (module) => changed.has(module.source) || module.related.some((path) => changed.has(path)),
  );
}

function runProcess(cwd, args, { coverageDirectory, timeoutMs }) {
...
async function copyProject(root, destination) {
  for (const path of ['examples', 'lib', 'test', 'package.json']) {
    await cp(join(root, path), join(destination, path), { recursive: true });
  }
}

async function coveredOffsets(coverageDirectory, sourcePath) {
...
export async function runModule(root, module, timeoutMs) {
...
export async function writeReports(root, mode, report) {
  ...
  for (const module of report.modules) {
    ...
    for (const status of ['killed', 'survived', 'timeout', 'no-coverage']) {
      const results = module.results.filter((result) => result.status === status);
