...
import { EventDroppedError, EventSenderClosedError } from '../../lib/framework/errors.js';
...
function destroyedRequest(address, path) {
  const responseReceived = deferred();
  const closed = deferred();
  const request = http.get({ ...address, path, agent: false }, (response) => {
...
test('close ждёт settlement HTTP-handler после уничтожения response и разрешает ему push', async (t) => {
  ...
  const application = new Application({
    http: { shutdownTimeout: 100, onError: (error) => errors.push(error) },
  });
  t.after(async () => {
  ...
  const earlyResult = await Promise.race([
    closing.then(() => 'closed'),
    new Promise((resolve) => setTimeout(resolve, 20, 'waiting')),
  ]);
...
test('push из HTTP-handler после forced transport cutoff получает EventSenderClosedError', async (t) => {
  const handlerStarted = deferred();
...
test('close опустошает event mailbox до закрытия JobRunner', async (t) => {
...
test('forced event shutdown отменяет active, наблюдает pending drops и ловит late rejection', async (t) => {
  ...
  const application = new Application({
    events: {
      handlerTimeout: 30,
      shutdownTimeout: 10,
      onError: (error, context) => observations.push({ error, context }),
    },
  });
  t.after(async () => {
  ...
  assert.ok(observations.every(({ error }) => error instanceof EventDroppedError));
  assert.ok(observations.every(({ context }) => Object.isFrozen(context)));
...
test('close ждёт WebSocket message-handler после закрытия сессии и разрешает ему push', async (t) => {
  const handlerStarted = deferred();
  const releaseHandler = deferred();
  const eventHandled = deferred();
  class Work {}
...
test('websocket.shutdownTimeout ограничивает зависший onDisconnect', async (t) => {
...
test('websocket.shutdownTimeout отменяет pending onConnect и освобождает handshake socket', async (t) => {
