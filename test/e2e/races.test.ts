...
import { JobRunner } from '../../lib/framework/JobRunner.ts';
...
function release(view: Int32Array, index: any) {
  Atomics.store(view, index, 1);
  Atomics.notify(view, index);
}

async function reached(view: Int32Array, index: any, label: any) {
...
function describeOrder(scenario: any, order: any, iteration: any) {
...
function waitForClose(emitter: any) {
  return new Promise<any>((resolve: any) => emitter.once('close', resolve));
}

function requestErrorHandled(request: any) {
  request.on('error', () => {});
  return request;
}
...
async function assertSingleSettlement(observation: any, expected: any, context: any) {
...
async function preparedRun(runner: any, options: any = {}, outcome: any = 'result') {
...
async function runJobRace(scenario: any, order: any, iteration: any) {
...
async function runCloseRace(scenario: any, order: any, iteration: any) {
...
test(
  ...
  { timeout: 20_000 },
  async () => {
    const lateFailures: any[] = [];
    const onUnhandledRejection = (error: any) => lateFailures.push(error);
    const onUncaughtException = (error: any) => lateFailures.push(error);
...
async function cancelWhileReadingBody(iteration: any) {
  let handlerCalls = 0;
  ...
  try {
    ...
    const continued = new Promise<any>((resolve: any) => request.once('continue', resolve));
...
async function cancelDuringHandler(iteration: any) {
  ...
  try {
    const request = requestErrorHandled(
      nodeHttp.get({ host: address.address, port: address.port, path: '/handler' }),
    );
    const closed = waitForClose(request);
...
async function cancelWhileWaitingForJob(iteration: any) {
  const control = barrier();
  ...
  try {
    const request = requestErrorHandled(
      nodeHttp.get({ host: address.address, port: address.port, path: '/job' }),
    );
    const closed = waitForClose(request);
...
async function cancelWhileWritingResponse(iteration: any) {
  ...
  try {
    ...
    requestErrorHandled(
      nodeHttp.get(
        { host: address.address, port: address.port, path: '/response' },
        (response: any) => {
          response.pause();
