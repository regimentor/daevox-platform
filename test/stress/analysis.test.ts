...
import { classifySteps } from './analysis.ts';
import { createShutdownChaosPlan, createStressConfig } from './config.ts';
...
test('shutdown chaos seed reproduces producer decisions', () => {
  ...
  assert.ok(first.every(({ closeDelayMs }: any) => closeDelayMs >= 0 && closeDelayMs <= 2));
});
...
test('stress harness records exact application-event throughput accounting', async (t: any) => {
