function exceedsRatio(value, baseline, ratio, floor = 0) {
  return value > Math.max(baseline, floor) * ratio;
}

function degraded(step, baseline, thresholds) {
  return (
...
export function classifySteps(steps, { recovery, ...thresholds }) {
  if (steps.length === 0) {
    return { firstDegradedStep: null, lastStableStep: null, recovery: 'not-measured' };
  }
  const baseline = steps[0];
  const observedWorkers = Math.max(0, ...steps.map((step) => step.workers ?? 0));
  const firstDegradedStep = steps.findIndex((step) => degraded(step, baseline, thresholds));
  const recovered =
