import { REGRESSION_THRESHOLDS } from './config.js';

function increase(current, baseline) {
  if (baseline === 0) return current === 0 ? 0 : Infinity;
  return (current - baseline) / baseline;
}

function drop(current, baseline) {
  if (baseline === 0) return 0;
  return (baseline - current) / baseline;
}

export function compareBenchmark(current, baseline, thresholds = REGRESSION_THRESHOLDS) {
