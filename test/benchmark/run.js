...
import { benchmarkConfig, REGRESSION_THRESHOLDS, SCHEMA_VERSION } from './config.js';
...
function runProfile(name, profileConfig, phases, injectedDelayMs) {
  const script = fileURLToPath(new URL('./profile-process.js', import.meta.url));
...
function summary(artifact, outputPath) {
  const lines = [
  ...
  for (const [name, profile] of Object.entries(artifact.profiles)) {
...
async function main() {
  ...
  const operationalFailures = Object.entries(profiles)
    .filter(([, profile]) => profile.metrics.errorRate > 0 || profile.metrics.successes === 0)
    .map(
      ([name, profile]) =>
