...
export function createShutdownChaosPlan(seed: any, iterations: any) {
...
export function createStressConfig(overrides: any = {}) {
  ...
  const normalizedConcurrency = stepConcurrency.map((value: any) =>
    positiveInteger(value, 'step concurrency'),
  );
  if (normalizedConcurrency.some((value: any) => value > limits.maxConcurrency)) {
  ...
  return {
    ...
    steps: normalizedConcurrency.map((concurrency: any) => ({
      concurrency,
