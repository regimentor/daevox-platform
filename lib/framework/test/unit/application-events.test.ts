...
function nextWebSocketMessage(socket: any) {
  return new Promise<any>((resolve: any, reject: any) => {
...
test('mailbox сохраняет FIFO одного listener и не блокирует другой listener', async (t: any) => {
...
test('mailbox запускает не более одного handler за один setImmediate', async (t: any) => {
...
test('push синхронно отклоняет неверный адрес, DTO и переполненный mailbox', async (t: any) => {
  class Work {}
  class Other {}
  ...
  class TriggerController extends HttpControllerBase {
    ...
    trigger() {
      const invalidCalls = [
        ...
        () => this.events.push({ listener: 'strict' } as any, new Work()),
        () =>
          this.events.push(
        ...
        () => this.events.push({ listener: 'strict', event: 42 } as any, new Work()),
      ...
      const invalid = invalidCalls.map((call: any) => {
        try {
          call();
          return false;
        } catch (error) {
...
test('queueSize считает ожидающие события, но не active handler', async (t: any) => {
...
test('registry и push копируют адрес, но передают ту же ссылку DTO', async (t: any) => {
...
test('ошибки handler изолированы от HTTP и тот же listener продолжает FIFO', async (t: any) => {
  const finished = deferred();
  const errors: any[] = [];
  const counters: any[] = [];
  class Work {
...
test('ошибка handler без events.onError передаётся в console.error', async (t: any) => {
  const handlerError = new Error('unobserved listener failure');
  let errorLogged: any;
  const logged = new Promise<any>((resolve: any) => {
    errorLogged = resolve;
  });
  const consoleError = t.mock.method(console, 'error', (error: any) => errorLogged(error));
  class Work {}
  class Listener extends EventListenerBase {
...
test('запланированный mailbox не запускает отброшенное при forced shutdown событие', async (t: any) => {
  const dropped = deferred();
  const scheduledCallbacks: any[] = [];
  ...
  application.registerHttpController(Controller);
  const address = await application.listen({ port: 0 });
  (globalThis as any).setImmediate = (callback: any, ...arguments_: any[]) => {
...
test('handler timeout отменяет signal, но FIFO ждёт поздний settlement без второй ошибки', async (t: any) => {
  const timedOut = deferred();
  const releaseLateHandler = deferred();
  const secondFinished = deferred();
  const errors: any[] = [];
  let timeoutReason: any;
...
test('ошибка observer передаётся в console.error и не задерживает mailbox', async (t: any) => {
  const finished = deferred();
  const logged: any[] = [];
  const originalConsoleError = console.error;
  console.error = (error: any) => logged.push(error);
  t.after(() => {
  ...
  await new Promise<any>((resolve: any) => setImmediate(resolve));
  assert.deepEqual(
    logged.map((error: any) => error.message),
    ['sync observer', 'async observer'],
  );
});

test('ошибка конструктора listener делает запуск необратимо failed', async (t: any) => {
