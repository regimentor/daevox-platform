...
export function linearSlopePerMinute(samples: any, field: any) {
  ...
  const meanX = points.reduce((sum: any, point: any) => sum + point.x, 0) / points.length;
  const meanY = points.reduce((sum: any, point: any) => sum + point.y, 0) / points.length;
  const numerator = points.reduce(
    (sum: any, point: any) => sum + (point.x - meanX) * (point.y - meanY),
    0,
  );
  const denominator = points.reduce((sum: any, point: any) => sum + (point.x - meanX) ** 2, 0);
  return denominator === 0 ? 0 : numerator / denominator;
}

function monotonicGrowthRatio(samples: any, field: any) {
  if (samples.length < 2) return 0;
  let growth = 0;
...
function trendThreshold(samples: any, field: any, maximum: any, minimumRatio: any) {
...
function latencyThreshold(samples: any, percentileName: any, maximumRatio: any) {
  const baselineCount = Math.max(1, Math.floor(samples.length / 3));
  const baseline = percentile(
    samples.slice(0, baselineCount).map((sample: any) => sample.latencyMs[percentileName]),
    0.5,
  );
  const current = percentile(
    samples.slice(-baselineCount).map((sample: any) => sample.latencyMs[percentileName]),
    0.5,
  );
...
function resourceThreshold(baseline: any, finalResources: any, maxActiveHandleGrowth: any) {
  const limits = {
  ...
  const excess = Object.fromEntries(
    Object.entries(limits).map(([field, allowance]: any) => [
      field,
  ...
  return {
    excess,
    status: Object.values(excess).some((value: any) => value > 0) ? 'failed' : 'passed',
  };
}

export function analyzeSoak(
  samples: any,
  { baselineResources, finalResources, thresholds, warmupMs }: any,
) {
  const steady = samples.filter((sample: any) => sample.elapsedMs >= warmupMs);
  ...
  return {
    passed: Object.values(results).every((result: any) => result.status === 'passed'),
