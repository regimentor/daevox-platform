...
import { applyMutant, score, selectChangedModules } from './harness.ts';

test('mutation catalog has unique reproducible targets', async () => {
  const ids = new Set();
  for (const module of modules) {
    const source = await import('node:fs/promises').then(({ readFile }: any) =>
...
test('mutation score and changed-module mapping are deterministic', () => {
  ...
  assert.deepEqual(
    selectChangedModules(modules, ['test/unit/http-router.test.ts']).map(({ id }: any) => id),
    ['http-routing'],
  );
