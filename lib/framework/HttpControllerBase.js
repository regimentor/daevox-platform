...
export class HttpControllerBase {
  ...
  constructor(options) {
    ...
    if (
      ...
      Reflect.ownKeys(options).some((key) => !['events', 'jobRunner', 'websocket'].includes(key)) ||
 ...
 * исполнитель задач.
 * @property {WebSocketSender} [websocket] Application-wide WebSocket sender. / Общий sender
 * WebSocket-приложения.
 * @property {EventSender} events Application-wide event sender. / Общий sender внутренних событий.
 * @private
 */
