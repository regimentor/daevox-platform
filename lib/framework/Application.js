 ...
 *
 * @param {EventOptions} [options] Event configuration. / Конфигурация событий.
 * @returns {NormalizedEventOptions} Normalized configuration. / Нормализованная конфигурация.
 * @private
 */
function normalizeEventOptions(options = {}) {
  ...
  if (
    Reflect.ownKeys(options).some((key) => typeof key !== 'string' || !EVENT_OPTION_KEYS.has(key))
  ) {
  ...
  for (const key of ['queueSize', 'handlerTimeout', 'shutdownTimeout']) {
...
export class Application {
  ...
  /**
   * @type {Set<Promise<*>>} Unsettled user HTTP-handler operations. / Незавершённые операции
   * пользовательских HTTP-обработчиков.
   * @private
   */
  #activeHttpHandlers = new Set();
  /**
   * @type {Set<Function>} HTTP-handler settlement waiters. / Ожидающие settlement HTTP-handler.
   * @private
   */
  #httpHandlerWaiters = new Set();
  /**
   ...
   */
  #webSocketTransport;
  /** @type {WebSocketSender} HTTP-controller server-push facade. / Фасад server push HTTP-контроллеров. */
  #webSocketSender;
  /**
   * @type {EventListenerRegistry} Registered event-listener catalog. / Каталог зарегистрированных
   * слушателей событий.
   * @private
   */
  #eventListeners = new EventListenerRegistry();
  /**
   * @type {NormalizedEventOptions} Application-event options. / Параметры внутренних событий.
   * @private
   */
  #eventOptions;
  /**
   * @type {EventDispatcher} Application-event runtime. / Runtime внутренних событий.
   * @private
   */
  #eventDispatcher;
   ...
   */
  constructor({ jobs, http, websocket, events } = {}) {
  ...
  #waitForHttpHandlers() {
    ...
    return new Promise((resolve) => this.#httpHandlerWaiters.add(resolve));
  }
 ...
 * @property {EventOptions} [events] Application-event configuration. / Конфигурация внутренних событий.
 * @public
 */
 ...
 * @property {number} [queueSize=1000] Waiting events per listener mailbox. / Число ожидающих
 * событий в mailbox одного слушателя.
 * @property {number} [handlerTimeout=30000] Handler timeout in milliseconds. / Тайм-аут handler.
 * @property {number} [shutdownTimeout=30000] Mailbox drain timeout in milliseconds. / Тайм-аут
 * опустошения mailbox.
 * @property {ApplicationEventErrorHandler} [onError] Accepted-event error observer. / Наблюдатель
 ...
 *
 * @typedef {Object} NormalizedEventOptions
 * @property {number} queueSize Waiting-event capacity. / Ёмкость ожидающих событий.
 * @property {number} handlerTimeout Handler timeout. / Тайм-аут handler.
 * @property {number} shutdownTimeout Drain timeout. / Тайм-аут drain.
 * @property {ApplicationEventErrorHandler} [onError] Error observer. / Наблюдатель ошибок.
 * @private
 */
 ...
 *
 * @callback ApplicationEventErrorHandler
 * @param {*} error Original or framework error. / Исходная ошибка или ошибка фреймворка.
 * @param {ApplicationEventAddress} context Frozen event address without DTO. / Замороженный адрес
 * события без DTO.
 * @returns {void|Promise<void>} Ignored observer result. / Игнорируемый результат observer.
 * @public
 */
 ...
 * @property {number} [shutdownTimeout=30000] Handler and lifecycle shutdown timeout. / Тайм-аут
 ...
 * @property {string} clientId Final framework or application client identifier. / Итоговый
