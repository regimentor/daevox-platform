...
class WebSocketConnection {
  ...
  #onSocketError;
  /**
   * @type {() => void} Physical socket-close listener. / Обработчик физического закрытия socket.
   * @private
   */
  #onTransportClose;
  /**
  ...
  constructor(
    socket,
    { maxPayload, onClose, onMessage, onProtocolError, onSocketError, onTransportClose },
  ) {
    this.#socket = socket;
...
export class WebSocketTransport {
  /**
   * @type {Set<WebSocketConnection>} Physical transport connections. / Физические транспортные соединения.
   * @private
   */
  #connections = new Set();
  /**
  ...
  /**
   * @type {Set<Promise<*>>} Unsettled transport user operations. / Незавершённые transport-операции.
   * @private
   */
  #operations = new Set();
  /**
   * @type {Set<Function>} Operation settlement waiters. / Ожидающие settlement операций.
   * @private
   */
  #operationWaiters = new Set();
  /**
   * @type {Map<AbortController, Socket>} Pending handshake sockets. / Ожидающие handshake socket.
   * @private
   */
  #pendingUpgrades = new Map();
  /**
   * @type {EventSender} Controller event sender. / Sender внутренних событий контроллеров.
   * @private
   */
  #events;
  /**
   ...
   */
  constructor({ controllers, events, jobRunner, onError, options, sessionStore }) {
  ...
  attach(server) {
    server.on('upgrade', (request, socket, head) => {
      const operation = this.#upgrade(request, socket, head);
   ...
   *
   * @param {Promise<void>} serverClosing Shared server close operation. / Закрытие общего сервера.
   * @param {number} timeout Grace timeout in milliseconds. / Тайм-аут grace-периода.
   * @returns {Promise<void>} Settlement or forced cutoff. / Settlement или forced cutoff.
   * @private
   */
  async waitForSettlement(serverClosing, timeout) {
    let timer;
   ...
   *
   * @param {Promise<*>} operation Tracked operation. / Отслеживаемая операция.
   * @returns {void}
   * @private
   */
  #track(operation) {
  ...
  #waitForOperations() {
    ...
    return new Promise((resolve) => this.#operationWaiters.add(resolve));
  }
 ...
 * @property {() => void} onTransportClose Physical close listener. / Обработчик физического закрытия.
 * @private
 */
