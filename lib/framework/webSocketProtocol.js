 ...
 *
 * @param {*} body Candidate body. / Проверяемое тело.
 * @returns {boolean} Validation result. / Результат проверки.
 * @private
 */
export function isWebSocketMessageBody(body) {
  return isPlainObject(body) && !Object.hasOwn(body, 'error') && isCompatible(body);
}
...
export function encodeWebSocketMessage(
  ...
  if (!isWebSocketMessageBody(body)) {
 ...
 * @param {WebSocketErrorCode} code Protocol error code. / Код ошибки протокола.
