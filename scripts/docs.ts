...
async function sourceFiles() {
  return (await readdir(path.join(root, 'lib', 'framework')))
    ...
    .toSorted()
    .map((file: any) => path.join(root, 'lib', 'framework', file));
}

function documentation(args: any) {
...
function formatMarkdown(file: any) {
  const result = spawnSync(formatterCli, [file], { cwd: root, stdio: 'inherit' });
  if (result.error) throw result.error;
  if (result.status !== 0) process.exit(result.status ?? 1);
}

async function stripTrailingWhitespace(directory: string): Promise<void> {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    const file = path.join(directory, entry.name);
    if (entry.isDirectory()) await stripTrailingWhitespace(file);
...
async function generate(outputRoot: any) {
  ...
  await stripTrailingWhitespace(path.join(outputRoot, 'api'));
}
